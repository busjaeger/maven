...
public class ModelSelector {
    ...
    static interface Selector extends Predicate<Model> {
        @Override
        public boolean apply(Model model);
    }

    static class ArtifactIdSelector implements Selector {
    ...
    static class IdSelector extends ArtifactIdSelector {
    ...
    static abstract class FileSelector implements Selector {
    ...
    static class ProjectFileSelector extends FileSelector {
    ...
    static class ProjectDirSelector extends FileSelector {
    ...
    static Selector parseSelector(String string, File baseDir) {
        ...
        if (idx >= 0) {
            ...
            selector = groupId.length() == 0 ? new ArtifactIdSelector(artifactId) : new IdSelector(artifactId, groupId);
        } else {
    ...
    public Map<GA, ? extends Model> select(final File baseDir, final List<String> selectors, final Map<GA, ? extends Model> models) {
        return selectors.isEmpty() ? models : filterValues(models,
                or(transform(selectors, parseSelectorClosure(baseDir))));
    }
